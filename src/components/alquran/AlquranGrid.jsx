...
import { AlquranContext } from '../../context/AlquranContext';
import AlquranSingle from './AlquranSingle';
  ...
  return (
    <section className="py-5">
      ...
      <div className="mt-2">
        <div className="flex justify-between border-b border-primary-light dark:border-secondary-dark pb-3 gap-3">
          <div className="flex justify-between gap-2 w-full">
            ...
            <input
              onChange={(e) => {
                setSearchAlquran(e.target.value);
              }}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 mt-6 sm:gap-10">
        {searchAlquran
          ? searchAlquranByTitle.map((alquran) => (
              <AlquranSingle
                name={alquran.latin}
                number={alquran.surah_number}
                arabic={alquran.arabic}
                translation={alquran.translation}
                ayat={alquran.total_ayat}
                key={alquran.surah_number}
              />
            ))
          : alquran.map((alquran) => (
              <AlquranSingle
